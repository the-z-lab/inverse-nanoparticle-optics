function [] = Run_Design_TwoBody()
    ...
    % Run the design
    Design_Position_TwoBody(x_0, box, omega, E_0, eps_p, a, x_E, step_size, T, error_tol, outfile)
    
    %% Brute force parameter sweep
    
    % Initialize
    r = (6:-0.01:2)'; % separation values
    N_r = length(r);
    E_mag = zeros(N_r,1);
    p_mag = zeros(N_r,1);
    ...
    for i = 1:N_r
        ...
        % Analytic solution
        [p_an, E_an, ~, ~, ~, ~, ~] = AnalyticTwoBody(x, alpha(1), box, E_0, x_E);
    ...
    % Plot results
    file = load(outfile);
    colors = parula(length(file.r));
    s = scatter(file.r, file.E_ave, 64, colors, 'LineWidth', 2);
    
    % Add colorbar
    c = colorbar;
    c.Label.String = 'iteration';
    caxis([1 length(file.r)]);
